...
async def scrape_series_id(
    session: aiohttp.ClientSession, manga: str
) -> tuple[str, str, str]:
    ...
    Returns:
        >>> (name, url, _id)
    """
    encoded_title = urllib.parse.quote(manga)
...
async def is_series_completed(session: aiohttp.ClientSession, manga: str) -> bool:
    """Check if the series is completed or not."""
    name, url, _id = await scrape_series_id(session, manga)

    api_url = "https://api.mangaupdates.com/v1/series/{id}"

    async with session.get(api_url.format(id=_id)) as resp:
        if resp.status != 200:
            return None
