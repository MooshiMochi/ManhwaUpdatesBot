...
def test_chapter_list_links_premium_chapters_with_lock_only_when_premium() -> None:
    ...
    assert f"[Chapter 2](https://example.test/chapter-2) {emojis.LOCK}" in text
...
def test_info_view_links_latest_and_first_chapters_with_premium_lock() -> None:
    ...
    assert f"**Latest Chapter:** [Chapter 9](https://example.test/chapter-9) {emojis.LOCK}" in text
...
def test_update_notification_links_premium_chapter_with_lock_on_the_right() -> None:
    ...
    assert f"**New chapter:** [Chapter 9](https://example.test/chapter-9) {emojis.LOCK}" in text
...
def test_bookmark_chapter_markdown_links_premium_chapters_with_lock() -> None:
    assert (
        ...
        == f"[Chapter 9](https://example.test/chapter-9) {emojis.LOCK}"
    )
...
def test_info_command_renders_dedicated_chapter_urls_when_info_payload_lacks_urls() -> None:
    ...
    async def _run() -> None:
        ...
        assert (
            f"**Latest Chapter:** [Chapter 9](https://example.test/chapter-9) {emojis.LOCK}" in text
        )
